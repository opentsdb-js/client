...
Client.prototype.get = function( clbk ) {
	...
	var fcn = get.call(this, this.url(), function onData( error, data ) {
		if ( error ) {
			clbk( error );
			return;
		}
...
*	Specify headers to go with all queries/requests (ie custom token, custom headers)
*
* @param {Object} header - request-compliant headers config (https://www.npmjs.com/package/request#custom-http-headers)
* @returns {Client} Client instance
*/
Client.prototype.headers = function( headers ) {
	if ( typeof headers !== 'object' ) {
		throw new TypeError( 'headers()::invalid input argument. Input must be a request-compliant object' );
	}
